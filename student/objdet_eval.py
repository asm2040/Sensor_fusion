...
from misc.objdet_tools import compute_box_corners as compute
import torch
...
def measure_detection_performance(detections, labels, labels_valid, min_iou=0.5):
    ...
    for label, valid in zip(labels, labels_valid):
        ...
        if valid: # exclude all labels from statistics which are not considered valid
            ...
            x_l,y_l,w_l,l_l,yaw_l,z_l=label.box.center_x,label.box.center_y,label.box.width,label.box.length,label.box.heading,label.box.center_z
            ground_truth=compute(x_l,y_l,w_l,l_l,yaw_l)
            ...
            for detection in detections: 
                ...
                # print(detection)
                x_d,y_d,w_d,l_d,yaw_d,z_d=detection[1].numpy(),detection[2].numpy(),detection[5].numpy(),detection[6].numpy(),detection[7].numpy(),detection[3].numpy()
                four_coners_curr_dect=compute(x_d,y_d,w_d,l_d,yaw_d)
                # print(four_coners_curr_dect)
                # print("+++")all_positives
                ...
                polygon_gt=Polygon(ground_truth)
                polygon_detect=Polygon(four_coners_curr_dect)
                intersect=polygon_gt.intersection(polygon_detect).area
                uinon=polygon_gt.union(polygon_detect).area
                iou=(intersect/uinon)
                # print('ious: ',iou)
                # print("^^^^^^^")
    ...
    #######
    ####### ID_S4_EX2 END #######     
    # print(all_positives,false_negatives,false_positives)
    # print("^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^")
...
def compute_performance_stats(det_performance_all,configs_det):
    ...
    # pos_negs[i][0]=all_positives,pos_negs[i][1]=true_positives,pos_negs[i][2]=false_negatives,pos_negs[i][3]=false_positives

    # print(np.array(pos_negs))
